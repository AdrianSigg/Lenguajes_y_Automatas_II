...
public class TablasAl {
  ...
  */
  ...
  public TablasAl(String cadena, String[] split){
    this.cadena = cadena;
    this.split = split;
  }
    ...
    for (int i = 0; i < split.length; i++) {
      ...
      case "ID":
        ...
        if(automata.var.aceptacion || automata.con.aceptacion ||
          automata.let.aceptacion || automata.num.aceptacion || 
          automata.cad.aceptacion)
          symbolTable(split[i]);
        else errorTable(split[i]);
        break;
  ...
  void enumerations(){
    /*Se utilizan Enum's para representar las tablas generadas de una manera
    mas clara*/
    enumerationError = errorHashTable.elements();
    enumerationSymbol = symbolHashTable.elements();
    enumerationKeywords = keywordsHashtable.elements();
  ...
  void resultado(){
    ...
    if (!errorHashTable.isEmpty()) {
      System.out.println("\nTabla de Errores");
      while (enumerationError.hasMoreElements())
        System.out.println(enumerationError.nextElement());
    }
    ...
    if (!keywordsHashtable.isEmpty()) {
      System.out.println("\nPalabras reservadas");
      while (enumerationKeywords.hasMoreElements())
        System.out.println(enumerationKeywords.nextElement());
    }
